...
func GetPolicyToken() string {
	...
	condition = append(condition, global.ServerConfig.Oss.UploadDir)
	...
	h := hmac.New(func() hash.Hash { return sha1.New() }, []byte(global.ServerConfig.Oss.Secrect))
	io.WriteString(h, debyte)
	signedStr := base64.StdEncoding.EncodeToString(h.Sum(nil))

	var callbackParam CallbackParam
	callbackParam.CallbackUrl = global.ServerConfig.Oss.CallbackUrl
	...
	policyToken.AccessKeyId = global.ServerConfig.Oss.Key
	policyToken.Host = global.ServerConfig.Oss.Host
	policyToken.Expire = expire_end
	policyToken.Signature = string(signedStr)
	policyToken.Directory = global.ServerConfig.Oss.UploadDir
